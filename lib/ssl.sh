...
function enable_ssl() {
	...
	if [ "" != "$MAILADDR" ] && [ -e $CERTBOT ]; then
		...
		# K_set proxy configuration
		sed -i -e "s|^\(\s*ssl_certificate\s\+\)\S\+;|\\1${LETSENCRYPTDIR}/fullchain.pem;|" \
		       -e "s|^\(\s*ssl_certificate_key\s\+\)\S\+;|\\1${LETSENCRYPTDIR}/privkey.pem;|" /etc/proxy/${FQDN}_ssl.conf

		current_id=`mysql -upr0xy -pPasSw0rd -e "select proxy_id from proxy.pair where provision_name = '$FQDN'" | tail -n 1`
		let i=${current_id}-1
		[ $current_id -gt 0 ] && rsync -avzhe 'ssh -p1010' /etc/proxy/${FQDN}_ssl.conf root@proxy${current_id}:/etc/nginx/conf.d/
		[ $current_id -gt 0 ] && rsync -avzhe 'ssh -p1010' /etc/proxy/${FQDN}_ssl.conf root@proxy${proxy_pair[$i]}:/etc/nginx/conf.d/
		if [ $current_id -gt 0 ]; then
			rsync -avzhe 'ssh -p1010' /etc/letsencrypt/live/${FQDN} root@proxy${current_id}:/etc/letsencrypt/live/
			rsync -avzhe 'ssh -p1010' /etc/letsencrypt/archive/${FQDN} root@proxy${current_id}:/etc/letsencrypt/archive/
			...
			ssh -p1010 root@proxy${current_id} 'systemctl reload nginx'
			ssh -p1010 root@proxy${proxy_pair[$i]} 'systemctl reload nginx'
		fi
		# modify WP configure
		wp_replace_proto "http" "https" $FQDN
	fi
